...
//Might be what we need for sensor settings and readings settings
// volatile bool sensorStat[8] = {false, false, false, false, false, false, false, false} //either its an int or bool
int i;
boolean biner[8];

// ******************************** FREE RTOS ******************************** //
TaskHandle_t TaskUDPHandle, TaskMQTTHandle, TaskNavigasiHandle, TaskConveyorHandle, TaskIndikatorSistemrHandle, TaskLiveTracking;
...
int currentUnloadingState = LOW;                                      //TO make sure the unloading process works, the var needs to get updated without having to rely on the function getting called
                                                                      //first
volatile bool ConCon = false;                                         //Counting how many kanan actually gets detected
int8_t HomeChoose1 = 0, HomeChoose2 = 0, Porting1 = 2, Porting2 = 2;  //HOme choosing feature
...
void AGV_Loading() {
  ...
  // if (moveTrigger == true) stepper_Move();
...
void AGV_MoveToStation2() {
  ...
  if ((millis() - turnStartTime) >= 5000 && curStationState == true) {
    ...
    vTaskDelay(500 / portTICK_PERIOD_MS);  // added to see the transition of the case
...
void AGV_Unloading() {
  ...
  //OLD CODE :
  if (sta2_signal == true) stepper_Move();  // UDP comms to STA2 to make sure its ready to receive workpiece
...
void AGV_TurnRight() {
  ...
  if ((millis() - turnStartTime) >= 800 && (sensor[0] > nilaiSens || sensor[1] > nilaiSens) && sensor[7] < nilaiSens) {
    MotorStop();
...
void AGV_ReturnBackward_ST1() {
  ...
  if (TimerState2 >= 5 && yangKanan == true) {
    ...
    turnStartTime = millis();
  } else if (TimerState2 < 5 && yangKanan == true) {
    setMotor(30, 30);
  } else {
    line_follow(40);
  }
...
void AGV_TurnRight_ST1_1() {
  volatile bool yangKiri = ((sensor[0] > nilaiSens || sensor[1] > nilaiSens || sensor[2] > nilaiSens) == true && sensor[7] < nilaiSens);
  KendaliBacasensor();
  //Making sure its turning
  //Making sure it got the line
  //Trying out Ticker
  if ((millis() - turnStartTime) >= 900) {  // A WORKING TURNING RIGHT CONFIG ???
    ...
    //Try to adjust the turning motion) {
    //---------------------TRYOUTS-------------------//
    setMotor(60, -60);  // This might be the cause of the turning motion that also push the AGV forward a little more making the delay overshoot
  }
}
...
void AGV_ReturnStraight_ST1_1() {
  ...
  volatile bool yangTengahKanan = (sensor[0] < nilaiSens && (sensor[3] > nilaiSens || sensor[4] > nilaiSens || sensor[5] > nilaiSens) == true && sensor[7] >= nilaiSens);
  volatile bool yangKiri = (sensor[0] > nilaiSens && sensor[7] < nilaiSens);
  // This is the end of the line for the SEQUENCE
  if ((millis() - moveStartTime) >= 2500) {
    Autohomedelay1 = true;
  }


  if ((millis() - turnStartTime) >= 1500 && yangTengahKanan == true && Autohomedelay1 == true) {
    // TimUp = 0;
    KananAja++;
    HomeChoose1++;
    turnStartTime = millis();
  ...
  else if (HomeChoose1 >= Porting1 && yangTengahKanan == true) {
    MotorStop();
    ...
    //   setMotor(50, 50);
  } else if ((yangTengahKanan == true && HomeChoose1 < Porting1)) {
    setMotor(30, 30);
  } else {
...
void AGV_TurnRight_ST1_2() {
  KendaliBacasensor();
  volatile bool yangKiri = ((sensor[0] > nilaiSens || sensor[1] > nilaiSens) == true && sensor[7] < nilaiSens);
...
void AGV_ReturnAROUND_ST1toST2() {
  ...
  if (TimerState4 >= 30 && yangTengahKanan == true) {
    MotorStop();
    turnStartTime = millis();
...
void AGV_TurnLeft_ST1toST2_0_5() {
  KendaliBacasensor();
  volatile bool yangKiri = (sensor[7] < nilaiSens && sensor[0] > nilaiSens);
...
void AGV_TurnLeft_ST1toST2_0_5_2() {
  ...
  if ((millis() - moveStartTime) >= 1200 && yangKanan == true) {
    MotorStop();
    vTaskDelay(500 / portTICK_PERIOD_MS);
    turnStartTime = millis();
    moveStartTime = millis();
...
void AGV_ReturnStraight_ST1toST2_2() {
  ...
  volatile bool isSimpangKiri = ((sensor[0] > nilaiSens || sensor[1] > nilaiSens) && sensor[7] < nilaiSens);
...
void AGV_TurnLeft_ST1toST2_1() {
  ...
  if ((millis() - turnStartTime) >= 1200 && yangTengahKanan == true) {
...
void Task_MQTT(void *pvParameters) {
  ...
  for (;;) {  //This is where void loop lives, and most likely where we'll do modifications and adding new things here, call a function ?
    ...
    if ((millis() - JSON1time) >= 500) {
      JSON1time = millis();
      mqttClient.publish("agv2/monitor/livetrack", JSONdata1().c_str());
      mqttClient.publish("agv2/monitor/state", agvmqtt.c_str());
    }
    if ((millis() - JSON2time) >= 1000) {
      JSON2time = millis();
      mqttClient.publish("agv2/monitor/pid", JSONdata4().c_str());
    }
    if ((millis() - JSON3time) >= 1200) {
      JSON3time = millis();
      mqttClient.publish("agv2/monitor/alarm", JSONdata3().c_str());
    }
    if ((millis() - JSON4time) >= 1500) {
      JSON4time = millis();
      mqttClient.publish("agv2/monitor/normal", JSONdata2().c_str());
    }
...
void Task_Navigasi(void *pvParameters) {
  ...
  for (;;) {
    ...
    if (g_AGVCfg.currentState == HOME_IDLE) {
      ...
      itemCount = 0;  // trying to reset the itemcount
      TimerState = 0;
      TimerState2 = 0;
      TimerState3 = 0;
      TimerState4 = 0;
      ...
      agvmqtt = "AGV2_AUTO_HOME_ST1toST2";
      C_S_Jarak[0] = 0;
      C_S_Jarak[1] = 0;
      if (proxHome != true) {
        g_AGVCfg.currentState = HOME_IDLE;
      ...
      agvmqtt = "AGV2_MOVING_TO_STATION_1";
      Sensor_Distance();
    }
